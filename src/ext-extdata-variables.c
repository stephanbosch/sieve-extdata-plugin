...
bool vnspc_sieve_extdata_validate(struct sieve_validator *valdtr,
				  const struct sieve_variables_namespace *nspc,
				  struct sieve_ast_argument *arg,
				  struct sieve_command *cmd,
				  ARRAY_TYPE(sieve_variable_name) *var_name,
				  void **var_data, bool assignment);
bool vnspc_sieve_extdata_generate(const struct sieve_codegen_env *cgenv,
				  const struct sieve_variables_namespace *nspc,
				  struct sieve_ast_argument *arg,
	...
	bool assignment)
{
	...
	if (array_count(var_name) != 2) {
		sieve_argument_validate_error(
			valdtr, arg, "extdata: "
			"invalid variable name within extdata namespace: "
	...
	if (name_element->num_variable >= 0) {
		sieve_argument_validate_error(
			valdtr, arg, "extdata: "
			"invalid variable name within extdata namespace 'extdata.%d': "
			"encountered numeric variable name",
			name_element->num_variable);
		return FALSE;
	}
	...
	if (assignment) {
		sieve_argument_validate_error(
			valdtr, arg, "extdata: "
			"cannot assign to extdata variable extdata.%s",
			variable);
		return FALSE;
	}
	...
	string_t **str_r)
{
	...
	if (!sieve_binary_read_string(renv->sblock, address, &var_name)) {
		sieve_runtime_trace_operand_error(
			renv, oprnd, "extdata variable operand corrupt: "
			"invalid name");
		return SIEVE_EXEC_BIN_CORRUPT;
	}

	if (str_r !=  NULL) {
		ext_value = ext_extdata_get_value(renv, this_ext,
						  str_c(var_name));
		if (ext_value == NULL) {
