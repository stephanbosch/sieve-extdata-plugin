...
static bool
tst_extdata_registered(struct sieve_validator *valdtr,
		       const struct sieve_extension *ext,
		       struct sieve_command_registration *cmd_reg);
static bool
tst_extdata_validate(struct sieve_validator *valdtr, struct sieve_command *tst);
static bool
tst_extdata_generate(const struct sieve_codegen_env *cgenv,
...
static bool
tst_extdata_operation_dump(const struct sieve_dumptime_env *denv,
			   sieve_size_t *address);
static int
tst_extdata_operation_execute(const struct sieve_runtime_env *renv,
			      sieve_size_t *address);
...
tst_extdata_validate(struct sieve_validator *valdtr, struct sieve_command *tst)
{
	...
	if (!sieve_validate_positional_argument(valdtr, tst, arg, "name",
						1, SAAT_STRING) )
		return FALSE;
